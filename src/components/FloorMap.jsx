      ...
      >
        ...
        {designMode && (
          <g style={{ pointerEvents: 'none' }}>
            <line x1={cursor.x} y1="0" x2={cursor.x} y2={BUILDING_HEIGHT} 
                  stroke="red" strokeWidth="1" strokeDasharray="4 2" opacity="0.5" />
            <line x1="0" y1={cursor.y} x2={BUILDING_WIDTH} y2={cursor.y} 
                  stroke="red" strokeWidth="1" strokeDasharray="4 2" opacity="0.5" />
...
const renderNode = (node, startNode, endNode, path, onNodeClick, designMode) => {
  ...
  if (node.type === 'stair') {
    return designMode ? <circle key={node.id} cx={node.x} cy={node.y} r="5" fill="orange" opacity="0.8"/> : null;
  }
    ...
    return (
      <g key={node.id} onClick={() => onNodeClick(node.id)} className="cursor-pointer hover:opacity-80 transition-opacity">
        {node.type === 'exit' ? (
          ...
          <>
            <rect x={node.x - 30} y={node.y - 30} width="60" height="60" fill={color} 
                  stroke={isPath ? '#3b82f6' : '#94a3b8'} strokeWidth={isPath ? 3 : 2} rx="4" />
            <text x={node.x} y={node.y} dy=".3em" textAnchor="middle" className="text-xs font-bold fill-slate-700 pointer-events-none">
              {node.label || node.id}
            </text>
          </>
        )}
